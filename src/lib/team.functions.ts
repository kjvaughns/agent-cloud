...
import { assertSameOrg, getMyPrimaryOrgId } from "@/lib/org-guard";
import { supabaseAdmin as _adminClient } from "@/integrations/supabase/client.server";
...
/**
 * Owner, or somebody the owner has given administrative rights.
 *
 * Mirrors what `getMyAccess` computes as `canSeeAgency`, resolved here rather
 * than round-tripping a server function from inside another one.
 */
async function administersAgency(userId: string): Promise<boolean> {
  ...
  const [{ data: org }, { data: perms }] = await Promise.all([
    ...
    admin.from("role_permissions").select("staff_is_admin, mgr_manage_permissions")
      .eq("profile_id", userId).eq("organization_id", orgId).maybeSingle(),
  ]);
  return org?.owner_id === userId
    || Boolean(perms?.staff_is_admin)
    || Boolean(perms?.mgr_manage_permissions);
}

/**
 * Everything one agent's dashboard shows, assembled from facts already queried
 * elsewhere rather than a new set of tables.
 *
 * ── Who may look ──
 * Yourself, always. Otherwise the target must be in your downline, or you must
 * administer the agency. Sharing an org is deliberately NOT enough: two agents
 * under different uplines share an org, and neither has any business reading
 * the other's book. `assertSameOrg` runs first as the outer bound, then the
 * downline/admin check narrows it.
 *
 * ── What it withholds ──
 * Contact details and producer identity — email, phone, NPN, licence state —
 * are legitimate agency oversight and are returned. SSN, date of birth and
 * banking are not, to anybody, including an owner. The product stopped
 * collecting them; nothing here quietly re-opens a read path to the rows that
 * remain.
 */
export const getAgentDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const target = data.agentId;

    if (target !== userId) {
      await assertSameOrg(userId, target);
      const [{ data: inDownline }, administers] = await Promise.all([
        (supabase as any).rpc("is_in_downline", { _upline: userId, _target: target }),
        administersAgency(userId),
      ]);
      if (!inDownline && !administers) {
        throw new Error("That agent is not in your team.");
      }
    }

    const [profileRes, producerRes, dealsRes, contractsRes, licencesRes, directRes] = await Promise.all([
      // Deliberately narrow. No ssn_last4, no date_of_birth, no banking — see
      // the docblock. npn_number is producer identity, which an owner may see.
      supabase.from("profiles")
        .select("id, first_name, last_name, email, phone, npn_number, created_at, last_active_at, status, upline_id, agency_level_id")
        .eq("id", target).maybeSingle(),
      supabase.from("producer_profiles")
        .select("resident_state, resident_license_number").eq("profile_id", target).maybeSingle(),
      // effective_date rides along so the charts can fall back for an imported
      // book that was never "posted". See lib/team/agent-dashboard.ts.
      supabase.from("policies")
        .select("id, policy_number, product, status, annual_premium, monthly_premium, posted_at, effective_date, client_id, carriers(name), clients(first_name, last_name)")
        .eq("agent_id", target).limit(5000),
      supabase.from("agent_commission_levels")
        .select("carrier_id, assigned_pct, commission_level, status, carriers(name)")
        .eq("agent_id", target),
      supabase.from("state_licenses")
        .select("state_code, status, expires_date").eq("agent_id", target),
      supabase.from("profiles")
        .select("id, first_name, last_name, agency_level_id").eq("upline_id", target),
    ]);

    if (profileRes.error) throw new Error(profileRes.error.message);
    if (!profileRes.data) throw new Error("No such agent.");

    const deals = (dealsRes.data ?? []).map((p: any) => ({
      id: p.id,
      ...
      client_name: [p.clients?.first_name, p.clients?.last_name].filter(Boolean).join(" ") || null,
      carrier_name: p.carriers?.name ?? null,
    }));

    // Each direct report's own numbers, for the drill-down cards. One batched
    // query rather than a dashboard call per person.
    const directIds = (directRes.data ?? []).map((d: any) => d.id);
    let directStats: Record<string, { deals: number; premium: number }> = {};
    if (directIds.length > 0) {
      const { data: theirs } = await supabase.from("policies")
        .select("agent_id, annual_premium").in("agent_id", directIds);
      for (const row of theirs ?? []) {
        const s = directStats[row.agent_id] ?? { deals: 0, premium: 0 };
        s.deals += 1;
        s.premium += Number(row.annual_premium ?? 0) || 0;
        directStats[row.agent_id] = s;
      }
    }
    ...
    return {
      ...
      directReports: (directRes.data ?? []).map((d: any) => ({
        ...d,
        deals: directStats[d.id]?.deals ?? 0,
        premium: directStats[d.id]?.premium ?? 0,
      })),
      isSelf: target === userId,
    };
  });
