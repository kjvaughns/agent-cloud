...
import { FRAME_W, type Anchor, type FrameRect, frameOf } from "./lib/anchors";
 ...
 * monotonically through all eight beats, and each screen slides to meet it.
 */
export const stageForCardY = (
  screen: ScreenKey,
  row: FrameRect,
  cardY: number,
  opts: { fit: number; zoom: number; pull: number; anchor: Anchor; scope?: ScreenScope },
): Stage => {
  const scope = opts.scope ?? "mine";
  const midY = frameOf(screen, scope).h / 2;
  const qy =
