...
import { FRAME_W, type Anchor, centerOf, frameOf } from "./anchors";
...
export type Stage = {
  screen: ScreenKey;
  /**
   * Which scope the screen is rendered in.
   *
   * Part of the stage rather than a render-time flag because it changes the
   * screen's *geometry* — the agency leaderboard is 100px taller and puts the
   * agent's own row 110px further down — and every transform below is relative
   * to the frame's middle. A scope the maths does not know about is a card that
   * slides off its row the instant the toggle flips.
   */
  scope: ScreenScope;
...
export const stage = (screen: ScreenKey, over: Partial<Stage> = {}): Stage => ({
  screen,
  scope: over.scope ?? "mine",
  fit: 1,
  zoom: 1,
  pull: 0,
  anchor: centerOf(screen, over.scope ?? "mine"),
  centerY: H / 2,
  translateX: 0,
  translateY: 0,
  ...over,
...
export const toCanvas = (s: Stage, p: Anchor): Anchor => {
  const mid = centerOf(s.screen, s.scope);
...
/** The frame's rendered height for a stage, which the scope can change. */
export const heightOf = (s: Stage) => frameOf(s.screen, s.scope).h;
