...
AS $function$
  ...
  select
    ...
    -- sets it by hand; posted_at is only the fallback audit stamp.
  ...
  where pol.agent_id in (select s from scope_agents where s is not null)
    -- An imported policy parked on the importer's id but held for another
    -- producer (assigned_to_email) is that producer's business. It still
    -- belongs to the agency/team book, but never to "mine".
    and (
      _scope <> 'mine'
      or pol.assigned_to_email is null
      or lower(pol.assigned_to_email) = lower(coalesce(pr.email, ''))
    )
