-- ---------------------------------------------------------------------------
-- A FOURTH ANSWER TO "WHOSE RECORDS AM I LOOKING AT?": TOTAL IMO
--
-- mine / team / agency all stop at the organisation boundary — deliberately;
-- is_in_downline carries a same-org constraint precisely so one agency's
-- rollups cannot wander into another's. An IMO is the one place that boundary
-- is supposed to open, and it opens on the parent's terms: a child agency is
-- counted only while its agency_relationships row is active with
-- include_production on. The rollup flows up, never down — a child resolving
-- 'imo' gets its own tree, and nothing here ever hands it the parent's.
--
--   imo_org_ids()   the caller's admin orgs plus, recursively, every child
--                   org whose relationship is active and included. Depth cap
--                   and cycle guard mirror is_in_downline's hardening — NOT
--                   the raw unbounded CTEs in the legacy dashboard RPCs.
--
--   scope_agent_ids('imo')   members of those orgs. Degrades to 'agency'
--                   (and further, by the existing rules) for anyone without
--                   an included child, so a stale ?scope=imo link narrows
--                   instead of erroring.
--
-- Also here: the owner participation toggles on organization_settings.
-- show_own_sales_in_feed / show_own_on_leaderboards let an IMO owner keep
-- their personal deals out of the shared feed and their line off the
-- leaderboards while still running the rollup. Default true — participation
-- is the norm; opting out is the deliberate act.
--
-- Depends on agency_relationships (20260814150000). Apply in order.
-- ---------------------------------------------------------------------------
...
as $$
  ...
  effective as (
    select case
      ...
      -- Degrade one step at a time: imo without children is agency; agency
      -- without adminship is team. A stale link narrows, never errors.
