-- Dashboard real-data fixes
-- 1) Per-agent monthly ALP goal (user-set, editable on the dashboard hero)
alter table public.profiles
  add column if not exists monthly_alp_goal numeric;

-- 2) get_dashboard_metrics corrections:
--    - team_prod / team_policies are DOWNLINE-ONLY (exclude the agent's own
--      production). Re-applies the intent of 20260610030000, which was
--      clobbered by 20260610032911.
--    - status_grid respects the requested range (was all-time).
--    - donut respects the requested range (was hardcoded trailing 30 days).
--    - active_downline counts the FULL recursive downline with status='active'
--      (was a raw count of direct reports regardless of status).
