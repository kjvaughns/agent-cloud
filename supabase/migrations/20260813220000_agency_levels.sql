-- One promotion ladder per agency. Carrier compensation is a mapping beneath
-- an agency level, not a second promotion ladder the owner has to recreate.
...
create policy agency_levels_read on public.agency_levels for select using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.organization_id = agency_levels.organization_id)
);
create policy agency_levels_write on public.agency_levels for all using (
  public.is_org_admin(organization_id)
) with check (public.is_org_admin(organization_id));

create policy agency_level_mappings_read on public.agency_level_carrier_mappings for select using (
...
create policy agency_level_mappings_write on public.agency_level_carrier_mappings for all using (
  public.is_org_admin(organization_id)
) with check (public.is_org_admin(organization_id));
