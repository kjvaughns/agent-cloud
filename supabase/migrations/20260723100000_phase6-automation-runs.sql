...
--
--   nova_automations stored what an agent WANTED to send. Nothing ever ran
--   them, and nothing recorded whether a given automation had already fired
--   for a given person — so the moment a worker existed it would re-send on
--   every sweep.
--
--   automation_runs is the idempotency ledger and the audit trail. Its unique
--   index is what makes the worker safe to run repeatedly.
--
-- Run after 20260722100000_phase5-reporting-notifications.sql.
-- ============================================================================
...
create table if not exists public.automation_runs (
  ...
  -- Who the message was about / addressed to.
  subject_type text not null check (subject_type in ('client','policy','agent')),
  subject_id uuid not null,

  -- The occurrence this run covers, e.g. '2026-07-29' for a birthday. Part of
  -- the idempotency key so next year's birthday is a separate, allowed run.
  ...
  -- Why a run did not send. 'blocked' means a consent or configuration gate
  -- refused it, which is a normal outcome, not an error.
  reason text,
...
-- The idempotency guarantee: one run per automation, per subject, per
-- occurrence, per channel. A re-run of the worker is a no-op.
...
-- An agent sees their own automation history; an org owner sees the agency's.
...
-- Inserts are service-role only: a run record is evidence of what the platform
-- did, and a user should not be able to write or forge one.
...
-- Bring nova_automations in line with the rest of the org-scoped tables so a
-- worker can select a single agency's automations without walking profiles.
