...
--
-- Three concerns that cut across the whole module:
--
--   submission templates  — the spreadsheet and email shapes each carrier wants
--   ready_to_sell_records — the cached agent x carrier answer that the roster
--                           renders, recomputed rather than derived per row
--   contracting_audit_log — a separate, append-only log. The existing
--                           audit_log has no record_type/record_id and is used
--                           for permission changes; overloading it would make
--                           both harder to read.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. SPREADSHEET TEMPLATES AND FIELD MAPPING
-- ---------------------------------------------------------------------------
...
create table if not exists public.contracting_spreadsheet_templates (
  ...
  name text not null,
  description text,
  -- The uploaded original. Never written to — exports are generated as new
  -- files so the agency always has the carrier's blank form intact.
  storage_path text,
  file_name text,
  sheet_name text,
...
create table if not exists public.contracting_field_mappings (
  ...
  -- Where it goes in the carrier's sheet.
  column_header text not null,
  column_index integer,
  -- Which Agent Cloud value fills it, resolved by the packet builder.
  source_key text not null,
  -- Used when source_key is 'literal', or when the source resolves empty.
  static_value text,
...
create table if not exists public.contracting_email_templates (
  ...
  name text not null,
  -- Which kinds of request this template is offered for. Empty means all.
  ...
  -- Attachments are opt-in per template and still require an explicit action
  -- at send time. Sensitive documents never ride along automatically.
...
-- Templates and mappings are agency configuration: readable by the contracting
-- workspace, writable by whoever manages carriers.
do $$
declare t text;
begin
...
--
-- Every packet, spreadsheet and email that leaves the system is recorded. This
-- is what makes "who sent the carrier what, and when" answerable months later.
-- ---------------------------------------------------------------------------
...
create table if not exists public.contracting_submissions (
  ...
  -- What was produced. Kept as a snapshot so a later profile edit does not
  -- rewrite history.
  ...
  -- Set when a human confirms they completed the external step.
...
-- 4. READY TO SELL
--
-- One row per agent x carrier, recomputed by the engine. A view would have to
-- re-derive licences, appointments, documents and writing numbers for every
-- row on every render; a roster of 200 agents across 15 carriers is 3,000
-- evaluations per page load.
-- ---------------------------------------------------------------------------
...
create table if not exists public.ready_to_sell_records (
  ...
  -- The states the agent can actually sell in for this carrier: licensed,
  -- appointed and with a writing number that covers them.
...
-- Read only to clients: this table is a computation, and letting a client
-- write it would let an agent declare themselves ready to sell.
...
--
-- Append only, and readable only by those explicitly granted it. Insert is
-- service_role only: an audit trail a client can write is not an audit trail.
-- ---------------------------------------------------------------------------
...
create table if not exists public.contracting_audit_log (
  ...
  record_id uuid,
  -- The producer the action concerns, when different from the actor.
...
--
-- Separate from the audit log because it is high volume and answers a
-- different question: who looked at or downloaded a producer's documents.
-- ---------------------------------------------------------------------------
