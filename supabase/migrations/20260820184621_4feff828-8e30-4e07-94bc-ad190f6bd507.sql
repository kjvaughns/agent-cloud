  ...
  IF TG_TABLE_NAME = 'call_logs' THEN
    ...
    LOOP
      ...
      IF (r.current_value + 1) >= r.target_value THEN
        ...
        v_type := CASE WHEN r.period IN ('daily','weekly','monthly','quarterly') THEN r.period::public.challenge_type ELSE 'daily'::public.challenge_type END;
        INSERT INTO public.trophies(agent_id, challenge_id, type, organization_id) VALUES (v_agent, r.id, v_type, r.organization_id);
      END IF;
    END LOOP;
