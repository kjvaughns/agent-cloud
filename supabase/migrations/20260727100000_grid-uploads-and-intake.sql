...
--
--   1. Agencies own their carrier comp grids. commission_grids had no owner
--      column, so every agency shared one global set — an agency could not
--      enter its own contract levels, which is what makes the commission
--      forecast right or wrong.
--   2. document_intake — bulk upload of carrier reports and statements for AI
--      to read and route.
--
-- Run after 20260726100000_client-banking-card.sql.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. AGENCY-OWNED COMMISSION GRIDS
--
--    organization_id NULL keeps the existing rows working as a shared default
--    set. An agency's own rows shadow the defaults for that carrier.
-- ---------------------------------------------------------------------------
...
-- Read the shared defaults plus your own agency's rows; write only your own.
...
-- Uploaded source documents, kept so a grid can be traced back to the page it
-- came from and re-read if the extraction was wrong.
...
--
--    One row per uploaded file. AI classifies it, summarizes it, and proposes
--    an action; a human confirms before anything is applied. Nothing here
--    writes to policies or commission_schedule on its own — an AI misread
--    should never silently change the book.
-- ---------------------------------------------------------------------------
