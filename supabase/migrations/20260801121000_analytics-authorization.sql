-- ---------------------------------------------------------------------------
-- Two authorization fixes on the analytics RPCs.
--
-- 1. get_carrier_breakdown takes an agent id and never checks it.
--
--    It is SECURITY DEFINER, so it runs with the definer's rights and RLS does
--    not apply. The `scope` CTE reads, in full:
--
--        SELECT id FROM team WHERE _agent IS NULL
--        UNION
--        SELECT _agent WHERE _agent IS NOT NULL
--
--    Any authenticated user could pass any profile id — including one in a
--    different organisation — and read that person's carrier production,
--    deal counts and premium. The sibling function get_agent_analytics, added
--    in the same migration, does guard its _agent parameter; this one was
--    missed.
--
--    The fix is that same guard, so the two agree.
--
-- 2. get_agent_analytics accepts a global admin as authorization.
--
--    has_role(auth.uid(), 'admin') is not scoped to an organisation, so
--    holding the admin role anywhere granted read access to any agent's
--    analytics everywhere. That is the global-admin bypass the org-isolation
--    work deliberately removed elsewhere; this call site kept it. Narrowed to
--    an admin of the *subject's* organisation.
--
-- Function bodies are otherwise reproduced verbatim. The unhardened recursive
-- CTEs in these functions are a separate problem and are not touched here.
-- ---------------------------------------------------------------------------
...
DECLARE
  ...
  result jsonb;
BEGIN
  -- Refuse outright rather than silently returning an empty breakdown, so a
  -- caller asking for someone they may not see gets told, not misled.
