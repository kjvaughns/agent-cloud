/**
 * A believable agency, seeded from nothing.
 *
 *   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... \
 *     npx esbuild scripts/seed-demo.ts --bundle --platform=node --format=esm \
 *       --outfile=.seed.mjs && node .seed.mjs --org <uuid> [--clear] [--dry-run]
 *
 * Or, once the package script is wired: `npm run seed:demo -- --org <uuid>`.
 *
 * This one artifact backs four things: the public demo org, the sample data a
 * new agency can start with, the ghost rows in empty states, and the fixtures
 * the end-to-end tests will run against. It is worth more care than a fixture
 * file usually gets.
 *
 ...
 * admin API regardless. This uses the service-role client and writes the same
 * shapes the application writes, honouring the same invariants — but it is not
 * an RLS test and should not be mistaken for one. The end-to-end suite in
 * Prompt 15 is where policies get exercised by a real session.
 */
...
if (!ORG_ID) {
  console.error("Usage: seed-demo --org <organization uuid> [--clear] [--dry-run]");
  process.exit(1);
}
...
async function clearSampleData(orgId: string) {
...
if (DRY_RUN) {
  ...
  console.log(`Would seed org ${ORG_ID}: ${CARRIERS.length} carriers, 55 clients, ~52 policies,`);
  console.log("commissions computed from the comp grid, 4 contracting requests, 5 tasks.");
  process.exit(0);
}

await clearSampleData(ORG_ID);
if (!CLEAR_ONLY) await seed(ORG_ID);
