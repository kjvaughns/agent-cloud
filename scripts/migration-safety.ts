...
const REVIEWED: Record<string, string> = {
  ...
  "src/lib/api/authenticate.server.ts:api_keys":
    "the key lookup checks its error and answers 500 without leaking why; before the table exists no key can match, so every API call is refused with a 401 rather than answering with somebody's data. Refusing is the safe direction for a credential check, and the endpoints are new — nothing in the product depended on them a release ago",
  "src/lib/api/authenticate.server.ts:api_key_usage":
    "the usage insert is wrapped in try/catch whose catch logs and returns, on the standing rule that recording a call must never be the thing that fails it. Before the table exists every call is being refused anyway, so the ledger has nothing to miss",
  "src/lib/api-keys.functions.ts:api_keys":
    "listApiKeys is the settings panel's own query and throws on error, which the panel renders as its error state — the honest outcome for a screen whose entire subject is a table that is not there yet. createApiKey and revokeApiKey throw for the same reason: an owner must never be told a key was issued or withdrawn when no row was written",
  "src/lib/api-keys.functions.ts:api_key_usage":
    "read without checking its error, deliberately: the call counts are decoration beside the key list, and a missing table leaves them at zero rather than emptying the panel that lists the keys themselves",
  "src/lib/dashboard.functions.ts:get_org_leaderboard":
    "agencyBoard() checks the rpc error and returns null on any failure, and the agency branch in getLeaderboardData falls through to the existing downline path when it does. A missing function answers PGRST202, which is an error rather than an empty result, so the degrade is the ordinary path and not a coincidence. In the window the board renders exactly what it renders today — the viewer's downline — which is narrower than intended but not wrong, and the reason is logged at console.warn rather than left silent. The same shape resolveScopeAgentIdsOrNone already uses for a missing scope RPC",
